...
class Generator(nn.Module):
  def __init__(self, inchannel, outchannel):
    ...
    for i in range(16):
        convs.append(AttResnetBlock(G0))
...
class AttResnetBlock(nn.Module):
  def __init__(self, Channels, kSize=3):
    super(ResBlock, self).__init__()
    self.channels = Channels
    self.relu = nn.ReLU()
  ...
  def forward(self, x):
    res_out = self.res(x)        # 残差卷积
    attn_out = self.cbam(res_out)  # 🔥 注意力处理
    return x + attn_out          # 残差连接
...
class NLayerDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, no_antialias=False):
        ...
        padw = 1
        if(no_antialias):
            sequence = [nn.Conv2d(input_nc, ndf, kernel_size=kw, stride=2, padding=padw), nn.LeakyReLU(0.2, True),SEBlock(ndf) ]
        else:
            sequence = [nn.Conv2d(input_nc, ndf, kernel_size=kw, stride=1, padding=padw), nn.LeakyReLU(0.2, True), SEBlock(ndf),Downsample(ndf)]
        nf_mult = 1
        nf_mult_prev = 1
