...
class NLCLModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--lambda_DisNCE', type=float, default=0, help='weight for Dis NCE loss')
        parser.add_argument('--lambda_MSE', type=float, default=0, help='weight for MSE loss')
        ...
        parser.add_argument('--nce_T', type=float, default=0.77, help='temperature for NCE loss') #0.07
        parser.add_argument('--num_patches_pos', type=int, default=8, help='number of patches per layer')
        parser.add_argument('--num_patches_neg', type=int, default=256, help='number of patches per layer')
...
#什么是nce损失哇？
        ...
        if self.isTrain:
            ...
            self.netGenSample = networks.define_F(opt.input_nc, opt.netFGen, opt.normG, not opt.no_dropout,
                                                  opt.init_type, opt.init_gain, opt.no_antialias, self.gpu_ids, opt)# 生成图像的样本提取器
            self.netAdvRainSample = networks.define_F(opt.input_nc, opt.netFAdvRain, opt.normG, not opt.no_dropout,
                                                      opt.init_type, opt.init_gain, opt.no_antialias, self.gpu_ids, opt) #用于雨图的对比学习样本提取器
            self.netAdvBackSample = networks.define_F(opt.input_nc, opt.netFAdvBack, opt.normG, not opt.no_dropout,
                                                      opt.init_type, opt.init_gain, opt.no_antialias, self.gpu_ids, opt) # 用于背景图的对比学习样本提取器
...
# 如果使用了NCE损失，就为 GenSample 模块创建优化器。
            ...
            self.optimizer_AdvBackSample = torch.optim.Adam(self.netAdvBackSample.parameters(), lr=self.opt.lr,betas=(self.opt.beta1, self.opt.beta2))
    ...
    def forward(self):
        ...
        (h,w) = self.O.shape[2:]
        self.pred_R = self.netRain(self.O) # 雨图网络预测的雨层图
        self.image_transferred = self.netBack(self.image_to_transfer) # 背景网络预测的背景图
        # torch.cuda.synchronize()
